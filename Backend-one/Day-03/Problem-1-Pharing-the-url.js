...
const server = http.createServer((req,res)=>{
    ...
    // This works. But JavaScript already has a built in way to do this —

    const number = Number(req.url.slice(1));
    // slice(1) removes the first character which is /. Same result. One line.

    // invalid url
    if(isNaN(number)) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: "Please provide a valid number" }));
        return;
    }
    ...
    if(number > gitHub.length) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: `Maximum ${gitHub.length} profiles available` }));
        return;
    }
  ...
  const arr = gitHub.slice(0, number);
    ...
    res.end(JSON.stringify(arr),null);
})
